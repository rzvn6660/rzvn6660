<p align="center">
  <img src="./IMG.png" alt="Mohammed Rizvin Banner" width="100%">
</p>
<h1 align="center">Hi, I'm Mohammed Rizvin 👋</h1>

<h3 align="center">
AI Engineer • LLMs • AI Agents • Voice AI • NLP • Computer Vision
</h3>

<p align="center">
Building production-focused AI applications using Large Language Models, Voice AI, OCR, and Backend Engineering.
</p>

<p align="center">
Open to AI Engineer • Applied AI Engineer • Generative AI Engineer opportunities
</p>

---
## 👨‍💻 About Me

- Building practical AI applications using Python, FastAPI, and modern AI frameworks.
- Passionate about LLMs, AI Agents, Voice AI, OCR, and Healthcare AI.
- Developing end-to-end AI systems from backend APIs to intelligent user experiences.
- Focused on writing clean, scalable, and production-oriented code.

- ## 🚀 Current Focus

- 🤖 AI Agents & LLM Applications
- 🎙️ Voice AI & Speech Recognition
- 📄 OCR & Document Intelligence
- 🏥 Healthcare AI
- ⚡ FastAPI & Docker

## 🌟 Featured Projects

### 🧠 [ORMA AI](https://github.com/rzvn6660/orma-ai)

Voice-first AI assistant for elderly care featuring multilingual conversations, medication reminders, memory assistance, and caregiver support.

**Tech:** Python • FastAPI • LLMs • Voice AI • OCR

---

### 💊 [Medi-fy](https://github.com/rzvn6660/Medi-fy)

AI-powered medicine label intelligence platform using OCR and LLMs to extract and explain medication information.

**Tech:** Python • EasyOCR • OpenCV • LLMs

---

### 🌍 [Multilingual AI](https://github.com/rzvn6660/Multilingual-AI)

Speech AI platform supporting multilingual communication and low-resource languages.

**Tech:** Python • Whisper • Coqui TTS • FastAPI

---

### 🌐 [Portfolio](https://github.com/rzvn6660/Rizvin_Portfolio)

Modern portfolio showcasing AI engineering projects, technical skills, and professional work.

**Tech:** React • JavaScript • CSS

## 🛠️ Tech Stack

### Languages
Python • JavaScript • SQL

### AI & Machine Learning
PyTorch • TensorFlow • Transformers • LangChain • Hugging Face • Scikit-learn

### Backend
FastAPI • REST APIs

### Computer Vision
OpenCV • EasyOCR

### Speech AI
Whisper • Coqui TTS

### Databases
SQLite • PostgreSQL

### Tools
Docker • Git • Linux • VS Code


## 📚 Currently Exploring

- Multi-Agent AI Systems
- Retrieval-Augmented Generation (RAG)
- Model Context Protocol (MCP)
- Production AI Deployment
- Open Source AI

- ## 📬 Connect

📧 **Email:** rizvinmk@gmail.com

💼 **LinkedIn:**  
https://linkedin.com/in/mohammed-rizvin-mk

🌐 **Portfolio:**  
https://rizvin-portfolio.vercel.app/


---

> **Building AI systems that transform ideas into practical solutions through LLMs, Voice AI, OCR, and intelligent automation.**
